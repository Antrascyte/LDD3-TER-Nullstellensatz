...
\underline{Démonstration} - \\ Donc $D(f)$ est bijection avec les idéaux premiers de $A$ qui ne contiennent pas $f$ par la remarque au dessus. $A[\frac{1}{f}]$ est une $k$- algèbre de type fini car $A$ l'est. Donc par le morphisme naturel  $ A & \to & A[\frac{1}{f}] $, les images réciproques d'idéaux premiers de $A[\frac{1}{f}]$ sont premiers dans $A$. Tout $D(f)$ contient au moins un idéal premier, donc $Spec(A)$ également.
